...
def train_steps(net, optimizer, buffer, device, n_steps, batch_size,
    ...
    for _ in range(n_steps):
        ...
        # policy entropy + top-1 prob: observability + optional anti-collapse reg
        probs = logp.exp()
        ...
        tot_v += vloss_item
        tot_e += float(entropy.item())
        tot_t1 += float(top1.item())
    return tot_p / n_steps, tot_v / n_steps, tot_e / n_steps, tot_t1 / n_steps
