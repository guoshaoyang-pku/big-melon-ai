...
class MCTS:
    def __init__(self, net, device, K=16, num_simulations=500,
                 ...
                 boundary_features=False, use_value_net=True,
                 leaf_value_mode="heuristic", leaf_safety_w=0.15,
                 leaf_fill_w=0.05, leaf_value_clip=0.5):
        self.net = net
        self.device = device
        self.K = K
        ...
        self.use_value_net = bool(use_value_net)
        self.leaf_value_mode = str(leaf_value_mode)
        self.leaf_safety_w = float(leaf_safety_w)
        self.leaf_fill_w = float(leaf_fill_w)
        self.leaf_value_clip = float(leaf_value_clip)
    ...
    def _leaf_value(self, vec, net_val):
        """Leaf bootstrap value used by backups.

        With ``use_value_net`` we trust the learned value head (classic
        AlphaZero). With it disabled we drop the learned value and use either a
        cheap bounded danger heuristic (``heuristic``) or 0 (``zero``). Either
        way Q stays a real reward-to-leaf estimate plus a small safety nudge, in
        the same normalised units as the merge rewards backed up by ``_backup``
        -- no double counting, since path rewards are added once and this term
        only estimates return *beyond* the leaf.
        """
        if self.use_value_net:
            return float(net_val)
        if self.leaf_value_mode == "zero":
            return 0.0
        # global-feature indices: 5 == top-y safety (lower fruits = safer),
        # 8 == fill fraction (more crammed = riskier); both live in the first 9
        # globals so they are stable regardless of boundary_features.
        v = self.leaf_safety_w * float(vec[5]) - self.leaf_fill_w * float(vec[8])
        if v > self.leaf_value_clip:
            return self.leaf_value_clip
        if v < -self.leaf_value_clip:
            return -self.leaf_value_clip
        return v
    ...
    def _expand_root_action(self, root, action):
        ...
        self._backup([(root, action)],
                     self._leaf_value(self._vec(child), vals[0]))
        ...
        while sims < self.num_simulations:
            ...
            while len(batch_leaves) < target:
                ...
                if leaf.is_terminal:
                    ...
                    _, v = self.net.infer_batch(self._vec(leaf)[None, :],
                                                self.device)
                    self._backup(path,
                                 self._leaf_value(self._vec(leaf), v[0]))
                else:
                    leaf.pending = True
            ...
            if batch_leaves:
                ...
                for lf, path, pr, v in zip(batch_leaves, batch_paths,
                    ...
                    self._backup(path, self._leaf_value(lf.vec, v))
